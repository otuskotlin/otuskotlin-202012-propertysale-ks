package ru.otus.otuskotlin.propertysale.backend.repository.cassandra.flat

import com.datastax.oss.driver.api.mapper.annotations.ClusteringColumn
import com.datastax.oss.driver.api.mapper.annotations.CqlName
import com.datastax.oss.driver.api.mapper.annotations.Entity
import com.datastax.oss.driver.api.mapper.annotations.PartitionKey
import ru.otus.otuskotlin.propertysale.be.common.models.flat.BePsFlatModel
import java.time.Instant

@Entity
data class FlatByNameCassandraDto(
    @PartitionKey(0)
    @CqlName(NAME)
    val name: String? = null,
    @ClusteringColumn(0)
    @CqlName(TIMESTAMP)
    val timestamp: Instant? = null,
    @ClusteringColumn(1)
    @CqlName(ID)
    val id: String? = null,
    @ClusteringColumn(2)
    @CqlName(NAME_INDEX)
    val nameIndex: String? = null,
) {

    companion object {
        const val FLATS_NAME_TABLE_NAME = "flats_by_name"
        const val ID = "id"
        const val NAME = "name"
        const val NAME_INDEX = "name_index"
        const val TIMESTAMP = "timestamp"

        fun of(model: BePsFlatModel) = of(model, model.id.id)

        fun of(model: BePsFlatModel, id: String) = FlatByNameCassandraDto(
            id = id.takeIf { it != BePsFlatModel.NONE.id.id },
            name = model.name.takeIf { it != BePsFlatModel.NONE.name },
            nameIndex = model.name.takeIf { it != BePsFlatModel.NONE.name },
            timestamp = Instant.now()
        )
    }
}
